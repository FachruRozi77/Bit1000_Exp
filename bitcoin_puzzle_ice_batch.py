#!/usr/bin/env python3
"""
Bitcoin Puzzle Transaction — HIGH-SPEED Batch Scanner (#71-#160)
====================================================================
Uses ice_secp256k1.so (iceland2k14) with point_sequential_increment
for batch key generation: ~3.5M keys/sec per CPU core.

Strategy:
1. Pick random starting key in puzzle range
2. Generate BATCH_SIZE sequential public keys in ONE C call
3. Hash all pubkeys and compare against pre-computed target hash160s
4. Only compute WIF/base58 on actual matches

Termux/Install:
    git clone https://github.com/iceland2k14/secp256k1.git
    # Place secp256k1.py and ice_secp256k1.so in same dir or PYTHONPATH

Usage:
    python3 bitcoin_puzzle_ice_batch.py [--workers N] [--batch N] [--puzzles 71,72,135]
"""
...
# ── Try to import ice_secp256k1 ──
try:
    import secp256k1 as ice
    ICE_AVAILABLE = True
except ImportError:
    ICE_AVAILABLE = False
    print("[!] ice_secp256k1 not found.")
    print("    git clone https://github.com/iceland2k14/secp256k1.git")
    print("    Then place secp256k1.py + ice_secp256k1.so in this directory.")
    sys.exit(1)
...
# ─────────────────────────────────────────────────────────────────────────
#  PRE-COMPUTE TARGET HASH160s for fast comparison
# ─────────────────────────────────────────────────────────────────────────

def _decode_address_to_hash160(addr: str) -> bytes:
    """Extract 20-byte hash160 from base58 Bitcoin address."""
    decoded = base58.b58decode(addr)
    return decoded[1:21]

# puzzle_num -> (low, high, target_hash160_bytes)
PUZZLE_DATA = {}
for num, (lo, hi, addr) in PUZZLES.items():
    PUZZLE_DATA[num] = (2 ** lo, 2 ** hi, _decode_address_to_hash160(addr))

# Build reverse lookup: hash160 -> puzzle_num
HASH160_TO_PUZZLE = {}
for num, (_, _, h160) in PUZZLE_DATA.items():
    HASH160_TO_PUZZLE[h160] = num

# Set of all target hash160s for fast membership testing
TARGET_HASH160_SET = frozenset(HASH160_TO_PUZZLE.keys())
...
# ─────────────────────────────────────────────────────────────────────────
#  WORKER — Batch sequential key generation with ice_secp256k1
# ─────────────────────────────────────────────────────────────────────────
def worker(worker_id: int, puzzle_nums: list, stats_dict, lock,
           result_lock, stop_flag: Value, batch_size: int):
    """
    For each cycle:
    1. Pick random puzzle and random starting key in its range
    2. Compute starting public key P = scalar_multiplication(start_key)
    3. Generate batch_size sequential pubkeys: P+G, P+2G, ... P+batch_size*G
       in ONE C call via point_sequential_increment
    4. Hash each pubkey and compare against targets
    """
    ...
    while not stop_flag.value:
        ...
        # Pick random start key, ensuring batch fits in range
        max_start = high - batch_size
        if max_start <= low:
            start_key = low
        else:
            start_key = rng.randint(low, max_start)

        # Generate starting public key
        P_start = ice.scalar_multiplication(start_key)

        # Generate batch_size sequential public keys in ONE C call!
        # Returns concatenated 65-byte uncompressed pubkeys
        batch_pubkeys = ice.point_sequential_increment(batch_size, P_start)

        found_batch = 0
        local_checked = batch_size

        # Process the batch: each pubkey is 65 bytes
        for i in range(batch_size):
            if stop_flag.value:
                break

            # Extract 65-byte pubkey
            pubkey = batch_pubkeys[i * 65:(i + 1) * 65]

            # Get hash160 from pubkey using ice (fast C function)
            hash160 = ice.hash160(pubkey)

            if hash160 in TARGET_HASH160_SET:
                # MATCH! Compute the actual private key and full details
                found_pk = start_key + i + 1
                addr = build_address(hash160)
                wif = build_wif(found_pk)
                puzzle_found = HASH160_TO_PUZZLE[hash160]

                with result_lock:
                    write_result(addr, found_pk, wif, puzzle_found)
                found_batch += 1
...
def draw_status(snap: dict):
    ...
    print("=" * 60)
    print("  BITCOIN PUZZLE #71-#160 — ICE BATCH SCANNER")
    print("  Using ice_secp256k1 point_sequential_increment")
    print("=" * 60)
    print(f"  Workers        : {snap.get('workers', 1)}")
    print(f"  Target puzzles : {snap.get('num_puzzles', 0)} unsolved addresses")
    print(f"  Batch size     : {snap.get('batch_size', 10000):,} keys/C-call")
...
# ─────────────────────────────────────────────────────────────────────────
def main():
    parser = argparse.ArgumentParser(description="Bitcoin Puzzle Scanner (#71-#160) — ICE BATCH")
    parser.add_argument("--workers", type=int,
                         default=max(1, multiprocessing.cpu_count() - 1),
                         help="Number of worker processes")
    parser.add_argument("--batch", type=int, default=100_000,
                         help="Keys per batch (default: 100,000). Larger = faster but more memory.")
    ...
    if not ICE_AVAILABLE:
        print("[!] ice_secp256k1 is required. See install instructions above.")
        sys.exit(1)
    ...
    print(f"[*] Batch size: {args.batch:,} keys per C call")
    print(f"[*] Using ice_secp256k1 point_sequential_increment")
    print(f"[*] Expected speed: ~3.5M keys/sec per CPU core")
    ...
    stats_dict["batch_size"] = args.batch
